        ...
        with sync_playwright() as p:
            for engine in ("chrome", "webkit"):
                ...
                print(engine, page.evaluate("""async () => {
                  ...
                  try {
                    check(toastDisplayDuration('저장했습니다.')===6000,'save duration');
                    check(toastDisplayDuration('연결 오류','error')===8000,'error duration');
                    check(toastDisplayDuration('저장 '+ '긴'.repeat(300))===12000,'long duration');
                    showToast('저장했습니다.');
                    ...
                    tick(5999);check(first.isConnected,'resume too early');
                    ...
                    button.blur();await Promise.resolve();tick(8000);
                    ...
                    tick(4000);showToast('저장 D');
                    check(refs.toastRegion.children.length===3,'duplicate toast');
                    tick(4000);check(queued.isConnected,'duplicate timer not renewed');
                    tick(4000);check(!queued.isConnected,'renewed toast never expires');
