        ...
        with sync_playwright() as p:
            for engine in ("chrome", "webkit"):
                ...
                print(engine, page.evaluate("""async () => {
                  ...
                  state.presenter.serviceId=service.id;
                  const nextService={id:'next-stability-fixture',type_id:'fixture'};
                  state.services=[service,nextService];state.selectedServiceId=service.id;
                  state.loadedWorshipServiceIds.delete(nextService.id);
                  canUseClientData=()=>true;
                  isPresenterOutputWindowOpen=()=>false;
                  let releaseHydration;
                  hydratePresenterServiceData=()=>new Promise(resolve=>{releaseHydration=()=>{state.loadedWorshipServiceIds.add(nextService.id);resolve(true)}});
                  preparePresenterService=id=>{state.presenter.serviceId=id};
                  renderServiceList=()=>{};renderPresenterDetail=()=>{};renderPresenterControlState=()=>{};
                  scrollPresenterBoardToTop=()=>{};scrollPresenterOutlineToActive=()=>{};syncBrowserHistory=()=>{};
                  const nextPreparation=prepareNextServiceFromPresenter(service.id,{nextServiceId:nextService.id});
                  check(state.selectedServiceId===service.id,'next service switched before hydration');
                  releaseHydration();await nextPreparation;
                  check(state.selectedServiceId===nextService.id && state.presenter.serviceId===nextService.id,'next service did not switch after hydration');
                  state.selectedServiceId=service.id;state.presenter.serviceId=service.id;
