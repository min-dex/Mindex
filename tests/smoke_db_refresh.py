...
def run(browser, url):
    ...
    try:
        ...
        print(page.evaluate("""async () => {
          ...
          state.dirty.forms=true;
          state.presenterPreparationDrafts={unused:'   '};
          worshipSetlistLeaderDrafts.set('unchanged',{original:'홍길동',value:'홍길동',saving:false});
          const reverted=document.createElement('div');
          reverted.className='svc-board-subgroup-control-item';
          reverted.dataset.inputStatus='modified';
          reverted.innerHTML='<input data-service-item-field="raw_title" value="original">';
          reverted.firstChild.dataset.initialValue='original';
          document.body.append(reverted);
          check(await MINDEX_DB_REFRESH.refresh()===true,'unchanged drafts or stale dirty flag blocked refresh');
          reverted.firstChild.value='actual edit';
          check(await MINDEX_DB_REFRESH.refresh()===false,'real deferred edit lost');
          reverted.firstChild.value='original'; reverted.dataset.inputStatus='error';
          check(await MINDEX_DB_REFRESH.refresh()===false,'failed save ignored');
          reverted.remove(); state.presenterPreparationDrafts={}; worshipSetlistLeaderDrafts.clear();
          state.forms[0].lyrics='actual unsaved lyrics'; state.dirty.forms=true;
          check(await MINDEX_DB_REFRESH.refresh()===false,'real model edit lost');
          state.forms[0].lyrics='보존된 가사';
          check(await MINDEX_DB_REFRESH.refresh()===true,'reverted model edit blocked');
