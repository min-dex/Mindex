...
(async()=>{try{await vm.runInContext(`(async()=>{
 state.calendarLoaded=true;state.calendarData=[{date:'2026-09-13',children_prayer:'어린이 담당',youth_prayer:'청소년 담당',young_adult_prayer:'청년 담당',youth_offering_prayer:'봉헌 담당'}];
 ...
 const payloads=['children','youth','young-adult'].map(type=>autoWorshipServicePayload({typeId:type,date:'2026-09-13'}));
 const created=await insertWorshipServicesWithCalendarAssignees(payloads);assert.equal(created.length,3);assert.equal(writes,3,'Batch writes should not multiply per service');assert.equal(stored.mindex_worship_elements.length,4);
