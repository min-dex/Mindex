...
assert "--presenter-output-bar-height: 20%;" in css
assert "--presenter-output-bar-height: 20%;" in preview_css
with sync_playwright() as p:
    ...
    for width, height in ((1920, 1080), (960, 540), (320, 180)):
        ...
        style = page.add_style_tag(content=css.replace("--presenter-output-bar-height: 20%;", "--presenter-output-bar-height: 17.5%;"))
        read = '''() => {
        ...
        assert abs(after["height"] / after["stage"] - 0.20) < 0.002, after
        for key in ("font", "weight", "padding"):
            assert before[key] == after[key], (key, before, after)
        if width == 960:
            page.screenshot(path="/tmp/mindex-caption-20.png")
        page.close()
    browser.close()
print("PASS 20% caption bar at 1920/960/320; font, weight and horizontal padding unchanged")
